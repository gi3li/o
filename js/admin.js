...
async function createDefaultSettings() {
    try {
        ...
        const defaultSettings = {
            ...
            logoUrl: 'https://i.ibb.co/zTdN34JZ/127eaa50-e46b-11f0-be13-f18061e34a08-1.png',
            ...
            productsHTML += `
                <div class="admin-product-card" data-id="${productId}">
                    ...
                    <div class="admin-product-info">
                        ...
                        <p><i class="fas fa-money-bill"></i> ${formatNumber(product.price || 0)} ${siteCurrency}</p>
                        <p><i class="fas fa-box"></i> المخزون: ${formatNumber(product.stock || 0)}</p>
        ...
        form.innerHTML = `
            <div style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px solid var(--border-color);">
                ...
                <div class="form-group">
                    ...
                    <small style="display: block; margin-top: 5px; color: var(--gray-color);">يجب أن يكون الرابط صورة مباشرة</small>
                </div>
...
// ======================== الإحصائيات المتقدمة وإدارة المخزون الذكية ========================

// تحميل الإحصائيات المتقدمة
async function loadAdvancedStats() {
    try {
        const ordersRef = window.firebaseModules.collection(adminDb, "orders");
        const ordersSnap = await window.firebaseModules.getDocs(ordersRef);
            ...
            statsContainer.innerHTML = `
                <div class="stats-grid">
                    <div class="stat-box">
                        ...
                        <h3>${formatNumber(totalRevenue)} ${siteCurrency}</h3>
                        <p>إجمالي المبيعات</p>
                    </div>
                    <div class="stat-box">
...
async function checkInventoryLevels() {
    try {
        const productsRef = window.firebaseModules.collection(adminDb, "products");
        const q = window.firebaseModules.query(productsRef, window.firebaseModules.where("stock", "<=", 5));
        const querySnapshot = await window.firebaseModules.getDocs(q);
        
        if (!querySnapshot.empty) {
            let lowStockProducts = [];
            querySnapshot.forEach(doc => {
                lowStockProducts.push(doc.data().name);
            });
            
            // إرسال إشعار للمدير
            sendBrowserNotification("تنبيه المخزون", {
                body: `المنتجات التالية أوشكت على النفاد: ${lowStockProducts.join(', ')}`
            });
        }
    } catch (error) {
...
// تحديث لوحة التحكم لتشمل الإحصائيات المتقدمة
...
async function loadAdminOrders() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            const date = order.createdAt?.toDate ? order.createdAt.toDate().toLocaleString('ar-EG') : 'غير محدد';
                ...
                <div class="order-card" id="order-${orderId}">
                    ...
                    <div class="order-footer">
                        ...
                        <div class="order-actions">
                            ...
                            <button class="delete-btn action-icon-btn" onclick="deleteOrder('${orderId}')">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
...
async function updateOrderStatus(orderId, newStatus) {
    try {
        ...
        loadAdminOrders(); // إعادة تحميل القائمة
    } catch (error) {
...
// دالة مساعدة للحصول على اسم الحالة بالعربية
